...
bool subghz_scene_receiver_on_event(void* context, SceneManagerEvent event) {
    ...
    if(event.type == SceneManagerEventTypeCustom) {
        ...
        case SubGhzCustomEventViewRepeaterStart:
            ...
            //Grab the number of bits and the length of each bit (Te)
            uint32_t tmpTe = 300;
            ...
            //Recalulate the length of the key.
            uint32_t tmpKeyLength = tmpTe * (tmpBits + 1) / 1000;
